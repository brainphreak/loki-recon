...
class WebUtils:
    ...
    def restore(self, handler):
        try:
            # cgi was removed in Python 3.13. Lazy-import so utils is loadable
            # on newer Python; the file-upload path is only hit at runtime.
    ...
    def serve_themes_list(self, handler):
        """List all available themes by scanning the themes/ directory."""
        themes_root = os.path.join(self.shared_data.currentdir, "themes")
        themes = []
        try:
            for name in sorted(os.listdir(themes_root)):
                ...
                if os.path.exists(tj):
                    try:
                        with open(tj, 'r') as f:
                            data = json.load(f)
                        display = data.get("theme_name", name)
                    except Exception:
                        pass
                themes.append({"id": name, "name": display})
        except FileNotFoundError:
            pass
        active = self.shared_data.config.get("theme", "bjorn")
        body = json.dumps({"active": active, "themes": themes})
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
        handler.send_header("Cache-Control", "no-cache")
        handler.end_headers()
    ...
    def api_v1_scan(self, handler):
        """POST /api/v1/scan {"action":"start"|"stop"|"pause"} — orchestrator control."""
        try:
            ...
            if action == 'start':
                self.start_orchestrator(handler)
                return
            if action in ('stop', 'pause'):
                self.stop_orchestrator(handler)
                return
            handler.send_response(400)
            handler.send_header("Content-Type", "application/json")
            handler.end_headers()
            handler.wfile.write(b'{"error":"action must be start|stop|pause"}')
        except Exception as e:
            handler.send_response(500)
            handler.send_header("Content-Type", "application/json")
            handler.end_headers()
    ...
    def set_theme(self, handler):
        """POST /api/theme {"theme": "<id>"} — switch theme and reload assets."""
        try:
            length = int(handler.headers.get('Content-Length', 0))
            raw = handler.rfile.read(length) if length else b'{}'
            payload = json.loads(raw.decode('utf-8') or '{}')
            ...
            if not new_theme:
                ...
                handler.send_header("Content-Type", "application/json")
                handler.end_headers()
                handler.wfile.write(b'{"error":"theme required"}')
                return

            themes_root = os.path.join(self.shared_data.currentdir, "themes")
            if not os.path.isdir(os.path.join(themes_root, new_theme)):
                handler.send_response(404)
                handler.send_header("Content-Type", "application/json")
                handler.end_headers()
                handler.wfile.write(json.dumps({"error": f"theme {new_theme} not found"}).encode('utf-8'))
                return

            self.shared_data.config['theme'] = new_theme
            self.shared_data.save_config()
            # Reload theme assets so the LCD renderer + UI palette pick up the change.
            # Order matters: defaults first (fonts → images), then theme
            # overrides those paths via load_theme().
            try:
                self.shared_data.load_fonts()
            except Exception as e:
                self.logger.debug(f"load_fonts after switch: {e}")
            try:
                self.shared_data.load_images()
            except Exception as e:
                self.logger.debug(f"load_images after switch: {e}")
            try:
                self.shared_data.load_theme()
            except Exception as e:
                self.logger.warning(f"load_theme after switch: {e}")
            # Commentaireia caches comments.json at boot — rebuild it so the
            # theme's per-status commentary lines start being used.
            try:
                loki = getattr(self.shared_data, 'loki_instance', None)
                if loki is not None:
                    from comment import Commentaireia  # noqa: PLC0415
                    loki.commentaire_ia = Commentaireia()
                    # Pull a comment from the NEW theme's comments.json immediately
                    # so the dialogue panel reflects the switch right away.
                    try:
                        status = getattr(self.shared_data, 'lokiorch_status', 'IDLE') or 'IDLE'
                        new_say = loki.commentaire_ia.get_commentaire(status)
                        if new_say:
                            self.shared_data.lokisay = new_say
                    except Exception as ce:
                        self.logger.debug(f"comment refresh: {ce}")
            except Exception as e:
                self.logger.debug(f"Commentaireia rebuild: {e}")
            # Force an immediate animation frame from the new theme so the
            # Display tab updates instantly, not on the next ~2s tick.
            try:
                self.shared_data.update_image_randomizer()
            except Exception:
                pass
            # Bust the LCD renderer's cached frame so the next /screen.png re-renders.
            try:
                import lcd_renderer  # noqa: PLC0415
                lcd_renderer._cache_frame = None  # type: ignore[attr-defined]
                lcd_renderer._cache_at = 0.0      # type: ignore[attr-defined]
            except Exception:
                pass
            ...
            handler.send_header("Content-Type", "application/json")
            handler.end_headers()
            handler.wfile.write(json.dumps({"status": "ok", "theme": new_theme}).encode('utf-8'))
        except Exception as e:
            self.logger.error(f"set_theme error: {e}")
            handler.send_response(500)
            handler.send_header("Content-Type", "application/json")
            handler.end_headers()
            handler.wfile.write(json.dumps({"error": str(e)}).encode('utf-8'))
    ...
    def clear_files(self, handler):
        try:
            # Clear logs, stolen data, scan results, vulnerabilities, zombies,
            # netkb + livestatus state — but NOT credentials.
            sd = self.shared_data
            command = f"""
            rm -rf {sd.logsdir}/* \
            ...
            """
            # Also zero in-memory + persisted attacks counter so Full Reset
            # truly clears the score, not just disk-derived counters.
            try:
                self.shared_data.attacksnbr = 0
                attacks_state = os.path.join(getattr(self.shared_data, 'datadir', '.'), 'attacks_count.json')
                with open(attacks_state, 'w') as f:
                    json.dump({'attacksnbr': 0}, f)
            except Exception as e:
                self.logger.debug(f"reset attacks counter: {e}")
    ...
    def clear_files_light(self, handler):
        try:
            ...
            # Keeps credentials, stolen files, vulnerabilities, zombies.
            sd = self.shared_data
            command = f"""
            rm -rf {sd.logsdir}/* \
              && rm -rf {sd.scan_results_dir}/* \
...
""")
    ...
    def save_configuration(self, handler):
        try:
            ...
            # If the theme changed, reload theme assets and bust the LCD cache
            # so the Display tab picks up the new look on the next /screen.png.
            # Order: fonts → images → theme overrides; Commentaireia caches
            # comments.json at boot so rebuild it too.
            try:
                self.shared_data.load_fonts()
                self.shared_data.load_images()
                self.shared_data.load_theme()
                ...
                self.logger.debug(f"theme reload after save: {reload_err}")
