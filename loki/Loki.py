...
class Loki:
    ...
    def is_wifi_connected(self):
        """True if the host has any non-loopback IPv4 address.

        Pager-original looked at wlan0cli / br-lan; on Pi/Linux/Docker the
        interface name varies. If we have a routable IP we can scan — that's
        all the orchestrator needs.
        """
        try:
            ...
            self.wifi_connected = any(
                line.strip() and ' lo ' not in line
