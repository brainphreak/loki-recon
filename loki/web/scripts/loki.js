...
var LokiTab = {
    ...
    init() {
        ...
        panel.innerHTML = '<div class="loki-panel">' +
            ...
            '<label for="lcd-theme" class="text-muted">Theme</label>' +
            '<select class="form-input" id="lcd-theme" style="max-width:220px"></select>' +
            '<label for="lcd-orient" class="text-muted">Orientation</label>' +
            '<select class="form-input" id="lcd-orient" style="max-width:160px">' +
            '<option value="270">Landscape</option>' +
        ...
        document.getElementById('lcd-theme').addEventListener('change', (e) => this.changeTheme(e.target.value));
        document.getElementById('lcd-orient').addEventListener('change', (e) => this.changeOrientation(e.target.value));
        this.loadThemes();
    ...
    async loadThemes() {
        try {
            var data = await App.api('/api/themes');
            var sel = document.getElementById('lcd-theme');
            if (!sel || !data || !data.themes) return;
            sel.innerHTML = data.themes.map(t =>
                '<option value="' + t.id + '"' + (t.id === data.active ? ' selected' : '') + '>' + t.name + '</option>'
            ).join('');
        } catch (e) {}
    },

    async loadOrientation() {
        try {
            var data = await App.api('/load_config');
            var sel = document.getElementById('lcd-orient');
            if (!sel || !data) return;
            var rot = String(data.screen_rotation != null ? data.screen_rotation : 270);
            // Snap arbitrary values to one of our two options.
            sel.value = (rot === '0' || rot === '180') ? '0' : '270';
        } catch (e) {}
    },

    async changeOrientation(rotation) {
        try {
            await App.post('/save_config', { screen_rotation: parseInt(rotation, 10) });
            this.refresh();
            App.toast('Orientation set to ' + (rotation === '0' ? 'Portrait' : 'Landscape'), 'success');
        } catch (e) {
            App.toast('Orientation change failed: ' + e, 'error');
        }
    },
    ...
    async changeTheme(themeId) {
        if (!themeId) return;
        try {
            await App.post('/api/theme', { theme: themeId });
            // Re-render the LCD scene with the new theme.
            this.refresh();
            // Reload the UI palette/title font from the new theme.
            if (window.App && typeof App.loadTheme === 'function') {
                App.loadTheme();
            }
            App.toast('Theme switched to ' + themeId, 'success');
        } catch (e) {
            App.toast('Theme switch failed: ' + e, 'error');
        }
    },
    ...
    async refresh() {
        ...
        try {
            ...
            if (ct.includes('octet-stream')) {
                ...
                // Adopt the PNG's actual dimensions so landscape themes
                // (480x222) don't get squashed into a portrait canvas.
                var blob = await resp.blob();
                var img = new Image();
                var self = this;
                img.onload = function() {
                    // PNG arrives already in display orientation, so canvas
                    // dimensions match the image and no CSS rotation is needed.
                    if (img.width !== self.canvas.width || img.height !== self.canvas.height) {
                        self.canvas.width = img.width;
                        self.canvas.height = img.height;
                        self.fbWidth = img.width;
                        self.fbHeight = img.height;
                        self.displayW = img.width;
                        self.displayH = img.height;
                        self.rotation = 0;
                        self.applyZoom();
                    }
