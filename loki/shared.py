...
class SharedData:
    ...
    def _migrate_legacy_data_layout(self):
        ...
        Pager layout (still valid, won't be touched if already migrated):
            <datadir>/output/{data_stolen,crackedpwd,vulnerabilities,zombies,scan_results}
            <datadir>/netkb.csv, livestatus.csv, attacks_count.json, api_token.json

        New layout:
            <datadir>/loot/{stolen,credentials,vulnerabilities,zombies}
            <datadir>/scans
            <datadir>/state/{netkb.csv, livestatus.csv, attacks_count.json, api_token.json}

        Migration is best-effort: any failure logs and continues.
        """
        import shutil
        legacy_output = os.path.join(self.datadir, 'output')
        moves = [
            (os.path.join(legacy_output, 'data_stolen'),       self.datastolendir),
            (os.path.join(legacy_output, 'crackedpwd'),        self.crackedpwddir),
            (os.path.join(legacy_output, 'zombies'),           self.zombiesdir),
            (os.path.join(legacy_output, 'vulnerabilities'),   self.vulnerabilities_dir),
            (os.path.join(legacy_output, 'scan_results'),      self.scan_results_dir),
            (os.path.join(self.datadir, 'netkb.csv'),          os.path.join(self.state_dir, 'netkb.csv')),
            (os.path.join(self.datadir, 'livestatus.csv'),     os.path.join(self.state_dir, 'livestatus.csv')),
            (os.path.join(self.datadir, 'attacks_count.json'), os.path.join(self.state_dir, 'attacks_count.json')),
            (os.path.join(self.datadir, 'api_token.json'),     os.path.join(self.state_dir, 'api_token.json')),
            (os.path.join(self.datadir, '.orchestrator_control'), self.orchestrator_control_file),
        ]
        ...
        for src, dst in moves:
            ...
            try:
                ...
                shutil.move(src, dst)
                logger.info(f"Migrated legacy data: {src} → {dst}")
            except Exception as e:
                logger.debug(f"Migration skip {src}: {e}")
        # If output/ is now empty, clean it up so it stops looking like live data.
        try:
            if os.path.isdir(legacy_output) and not os.listdir(legacy_output):
    ...
    def initialize_paths(self):
        ...
        # Resolution order: LOKI_DATA_DIR env var → config["data_dir"] → ~/.loki/data
        # The config-key path lets users change it from the UI without restarting
        # with a new env var. Old subdir layout (output/{data_stolen,...}) was
        # pager-style; new layout groups by purpose:
        #     <datadir>/
        #       loot/{stolen,credentials,vulnerabilities,zombies}
        #       scans/                (was output/scan_results)
        #       state/                (netkb.csv, livestatus.csv, attacks_count.json, api_token.json)
        #       logs/
        #       archives/
        #       input/
        env_dir = os.environ.get('LOKI_DATA_DIR')
        config_dir = None
        try:
            config_dir = (getattr(self, 'config', {}) or {}).get('data_dir')
        except Exception:
            config_dir = None
        self.datadir = (
            env_dir
            or config_dir
            or os.path.join(os.path.expanduser('~'), '.loki', 'data')
        )

        self.logsdir = os.path.join(self.datadir, 'logs')
        self.input_dir = os.path.join(self.datadir, 'input')
        self.archives_dir = os.path.join(self.datadir, 'archives')
        self.state_dir = os.path.join(self.datadir, 'state')
        self.loot_root = os.path.join(self.datadir, 'loot')

        # New canonical subdirs.
        self.crackedpwddir = os.path.join(self.loot_root, 'credentials')
        self.datastolendir = os.path.join(self.loot_root, 'stolen')
        self.zombiesdir = os.path.join(self.loot_root, 'zombies')
        self.vulnerabilities_dir = os.path.join(self.loot_root, 'vulnerabilities')
        self.scan_results_dir = os.path.join(self.datadir, 'scans')

        # Backwards-compat alias for clear_files shell-out (still valid — points to datadir).
        self.loot_dir = self.datadir
        # output_dir kept as an alias for any code that still references it.
        self.output_dir = self.loot_root

        # Control file for inter-process communication (webapp <-> orchestrator).
        self.orchestrator_control_file = os.path.join(self.state_dir, '.orchestrator_control')

        # One-time migration: pager-era data lived under output/, archives sat
        # at the datadir root, and state files were scattered. Move into the
        # new layout if old paths exist and new ones don't.
        self._migrate_legacy_data_layout()

        # Create the canonical tree.
        for d in [self.datadir, self.logsdir, self.input_dir, self.archives_dir,
                  self.state_dir, self.loot_root,
        ...
        self.netkbfile = os.path.join(self.state_dir, "netkb.csv")
    ...
    def _show_loading_screen(self, message="Loading..."):
        """Headless: forward loading message to the broker so the web UI can show it."""
        try:
            from display import broker
            broker.publish({"type": "loading", "message": message})
        except Exception as e:
            logger.debug(f"Could not publish loading event: {e}")
        ...
        try:
            ...
            for name in static_names:
                ...
                if path:
                    ...
                    # Skin-based theme system bakes these icons into main_bg.png
                    # instead of shipping individual PNGs (see commit 4aec12e).
                    # Demoted to debug so the log isn't spammed at every boot.
                    logger.debug(f"Static image not present (baked into bg): {name}")
